    ...
    const compileSass = () => {
      return (
        gulp
          ...
          // cssフォルダー以下に保存
          .pipe(gulp.dest("../fade-in/css", { sourcemaps: './' }))
...
//  htmlの自動リロード
 ...
 // SCSS配下のSCSSファイルへ変更があれば、反映される様にSCSSファイルを監視
    const watchSassFiles = () => { 
                                   gulp.watch("../fade-in/scss/**/*.scss", gulp.series(compileSass));
                                   gulp.watch("../fade-in/*.html", gulp.series(browserSyncReload));
                                  } 
